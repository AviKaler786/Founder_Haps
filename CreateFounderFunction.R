founderhap=function(Gmap,Gdat,ploidy = 2,inbred = F){
  
  #1a get genetic map 
  Gdat_c=read.csv(Gmap,header = T)
  Gdat_c$MarkerID=as.character(Gdat_c$MarkerID)
  Gdat0=read.vcf(Gdat) ##### read vcf file using readvcf function from Gformat (kaler's package)
  ...
  Gdat2=Gdat1[,c(ncol(Gdat1),10:(ncol(Gdat1)-1))]  ### subset the columns that we need. Remeber this is may vary depending upon  the number of genotype 
  ...
  score=sum(!(rownames(Gdat3)==as.character(Gdat_c$MarkerID))) ### this should give 0 
  print(score)
  ...
  if (score==0) {
    ...
    founderPop1 = newMapPop(genMap=genMapc1,haplotypes=haplotypes2)
    list(founderPop=founderPop1,haplotypes=haplotypes.a)
    
  }else{print("Marker Ids in Gmap dont match with Gdat")
  }
